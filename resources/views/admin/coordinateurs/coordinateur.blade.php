...
<div class="min-h-screen bg-gray-50">
    <div class="p-6 max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <button id="addCoordinateurBtn" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center space-x-2">
        ...
        <!-- Formulaire d'ajout -->
        <div id="addCoordinateurForm" class="bg-white rounded-lg shadow-md p-6 mb-6 hidden">
            <h2 class="text-xl font-semibold mb-4">Ajouter les informations d'un Coordinateur</h2>

            @if($usersCoordinateurs->count() > 0)
                <form method="POST" action="{{ route('gestion-coordinateurs.store') }}">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Sélection de l'utilisateur coordinateur -->
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <a href="{{ route('gestion-coordinateurs.index') }}" class="inline-block mt-4 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rôle Coordinateur</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($coordinateurs as $coordinateur)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ $coordinateur->user->nom }} {{ $coordinateur->user->prenom }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ $coordinateur->user->email }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('gestion-coordinateurs.show', $coordinateur) }}"
                                                ...
                                                <form method="POST" action="{{ route('gestion-coordinateurs.destroy', $coordinateur) }}"
                                                      class="inline-block" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer les informations de ce coordinateur ?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
...
document.addEventListener('DOMContentLoaded', function() {
    const addBtn = document.getElementById('addCoordinateurBtn');
    const addForm = document.getElementById('addCoordinateurForm');
