...
<div class="min-h-screen bg-gray-50">
    <div class="p-6 max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <button id="addAdminBtn" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center space-x-2">
                <i class="fas fa-plus"></i>
                <span>Ajouter Infos Admin</span>
            </button>
        </div>
        ...
        <!-- Formulaire d'ajout -->
        <div id="addAdminForm" class="bg-white rounded-lg shadow-md p-6 mb-6 hidden">
            <h2 class="text-xl font-semibold mb-4">Ajouter les informations d'un Administrateur</h2>

            @if($usersAdmins->count() > 0)
                <form method="POST" action="{{ route('gestion-admins.store') }}">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Sélection de l'utilisateur admin -->
                        <div class="md:col-span-2">
                            <label for="user_id" class="block text-sm font-medium text-gray-700 mb-1">Sélectionner l'administrateur</label>
                        ...
                        <!-- Rôle de l'admin -->
                        <div class="md:col-span-2">
                            <label for="role" class="block text-sm font-medium text-gray-700 mb-1">Rôle administrateur</label>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-600 mb-2">Aucun utilisateur avec le rôle "Administrateur" disponible</p>
                    <p class="text-sm text-gray-500">Vous devez d'abord créer des utilisateurs avec le rôle "Administrateur" dans la section Utilisateurs</p>
                    <a href="{{ route('gestion-admins.index') }}" class="inline-block mt-4 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rôle Admin</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($admins as $admin)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $admin->role == 'super admin' ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800' }}">
                                                {{ ucfirst($admin->role) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('gestion-admins.show', $admin) }}"
                                                ...
                                                <form method="POST" action="{{ route('gestion-admins.destroy', $admin) }}"
                                                      class="inline-block" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer les informations de cet administrateur ?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
